		...
		int32_t* runCounts)
{
	...
	for(i=0;i<readLength;i++) { /* read/rows */
		for(j=GETMAX(0, i - maxV);
			...
			// Fill in the cell
			AlignNTSpaceFillInCell(read, readLength, runCounts, reference, referenceLength, sm, matrix, i+1, j+1, maxH, maxV);
			//free(runCounts);
		}
	}
		...
		int32_t* runCounts)
{
	...
	for(i=1;i<endRowStepOne+1;i++) { /* read/rows */ 
		for(j=1;j<endColStepOne+1;j++) { /* reference/columns */
			AlignNTSpaceFillInCell(readAfterInsertion, readAfterInsertionLength, runCounts, reference, referenceLength, sm, matrix, i, j, readAfterInsertionLength, readAfterInsertionLength);
		}
	}
 ...
 * JSP: This handles the base cases for the alignment algorithm.
 * */
...
/*
 * JSP: This function does most of the work of the alignment.
 * It was modified to use gap open and gap extension functions.
 */
		...
		int32_t maxV) 
{
	...
	assert(0 < row);
	assert(0 < col);
	int32_t runLength = runCounts[row - 1]; /*JSP: added */
	/* Update deletion */
	if(maxV <= row - col) { // Out of bounds, do not consider
		...
		/* Deletion extension */
		matrix->cells[row][col].h.score[0] = matrix->cells[row][col-1].h.score[0] + ScoringMatrixGetGapExtensionScore(runLength, sm); // sm->gapExtensionPenalty;
		...
		if(matrix->cells[row][col].h.score[0] < matrix->cells[row][col-1].s.score[0] + ScoringMatrixGetGapOpenScore(runLength, sm)) { // sm->gapOpenPenalty) {
			matrix->cells[row][col].h.score[0] = matrix->cells[row][col-1].s.score[0] + ScoringMatrixGetGapOpenScore(runLength, sm); //  sm->gapOpenPenalty;
	...
	if(maxH <= col - referenceLength + readLength - row) { // Out of bounds do not consider
		...
		/* Insertion extension */
		matrix->cells[row][col].v.score[0] = matrix->cells[row-1][col].v.score[0] + ScoringMatrixGetGapExtensionScore(runLength, sm); //sm->gapExtensionPenalty;
		...
		if(matrix->cells[row][col].v.score[0] < matrix->cells[row-1][col].s.score[0] + ScoringMatrixGetGapOpenScore(runLength, sm)) { //sm->gapOpenPenalty) {
			matrix->cells[row][col].v.score[0] = matrix->cells[row-1][col].s.score[0] + ScoringMatrixGetGapOpenScore(runLength, sm); //sm->gapOpenPenalty;
