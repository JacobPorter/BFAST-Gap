		...
		FILE *fp) 
{
	...
	/* AS - optional field */
	/*JSP: Changed AS:i to AS:f*/
	if(entriesIndex < 0) { /* Unmapped */
		if(0>fprintf(fp, "\tAS:f:%d", INT_MIN)) {
			PrintError(FnName, NULL, "Could not write to file", Exit, WriteFileError);
		}
	}
	else {
		/*JSP: Print out a float instead. */
		//if(0>fprintf(fp, "\tAS:i:%d", (int32_t)a->ends[endIndex].entries[entriesIndex].score)) {
		if(0>fprintf(fp, "\tAS:f:%f", (float)a->ends[endIndex].entries[entriesIndex].score)) {
			PrintError(FnName, NULL, "Could not write to file", Exit, WriteFileError);
		}
	}
	/* MQ - optional field */
