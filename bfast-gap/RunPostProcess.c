...
#include <math.h>
#include <pthread.h>
/*JSP: Included the boolean type and the float type*/
//#include <stdbool.h>
#include <float.h>
		...
		FILE *fpOut)
{
	...
	/* JSP: print the scoring function */
	ScoringMatrixPrint(&sm, space);
...
/*JSP: This function needs to change for double score*/
static double getPairedScore(AlignedEntry *aOne,
                              AlignedEntry *aTwo,
                              int strandedness,
                              int positioning,
                              int avgMismatchQuality,
                              ...
                              PEDBins *b)
{
  /*JSP: Changed s to a double.*/
  double s = (aOne->score + aTwo->score);
  ...
  s -= (mismatchScore * -1.0 * log10( erfc(M_SQRT1_2 * numStd)) + 0.499);
               ...
               PEDBins *b)
{
  ...
  int32_t i, j;
  /*JSP: Changed bestScore and penultimateScore to doubles*/
      double bestScore;
	  int32_t bestNum;
      double penultimateScore;
	  int32_t penultimateNum;
      ...
      bestScore = -DBL_MAX;
      bestNum = 0;
      penultimateScore = -DBL_MAX;
      penultimateNum = 0;

      for(i=0;i<tmpA->ends[0].numEntries;i++) {
          for(j=0;j<tmpA->ends[1].numEntries;j++) {
              double score = getPairedScore(&tmpA->ends[0].entries[i],
                                             ...
                                             mismatchScore,
                                             b);
              if(!DoubleEquals(bestScore, score) && bestScore < score) {
              ...
              else if(DoubleEquals(bestScore, score)) {
                  bestNum++;
              ...
              else if(!DoubleEquals(penultimateScore, score) && penultimateScore < score) {
                  penultimateScore = score;
                  penultimateNum = 1;
              }
              else if(DoubleEquals(penultimateScore,score)) {
                  penultimateNum++;
              }
          }
      }
...
/*JSP: This file needs to be changed with double score */
		...
		PEDBins *b)
{
	...
	AlignedRead tmpA;
	int32_t i, j, k, ctr;
	/*JSP: made best a double*/
	double best;
	int32_t bestIndex, numBest;
			...
			switch(algorithm) {
				...
				case BestScoreAll:
					...
					for(j=0;j<tmpA.ends[i].numEntries;j++) {
						if(!DoubleEquals(best, tmpA.ends[i].entries[j].score) && best < tmpA.ends[i].entries[j].score) {
							best = tmpA.ends[i].entries[j].score;
							bestIndex = j;
							numBest = 1;
						}
						else if(DoubleEquals(best, tmpA.ends[i].entries[j].score)) {
							numBest++;
						}
					}
					...
					ctr=0;
					for(j=0;j<tmpA.ends[i].numEntries;j++) {
						if(DoubleEquals(best, tmpA.ends[i].entries[j].score)) {
							//fprintf(stderr, "best: %lf score: %lf\n", best, tmpA.ends[i].entries[j].score); // JSP: debug
							if(ctr != j) {
					...
					if (ctr != numBest) {
						fprintf(stderr, "ctr %d does not equal numBest %d.  best: %lf \n", ctr, numBest, best); //JSP: debug
					}
					assert(ctr == numBest);
		...
		for(i=0;i<tmpA.numEnds;i++) {
			for(j=k=0;j<tmpA.ends[i].numEntries;j++) {
				if(minimumMappingQuality <= tmpA.ends[i].entries[j].mappingQuality &&
						minimumNormalizedScore <= tmpA.ends[i].entries[j].score/ (double) AlignedEntryGetReadLength(&tmpA.ends[i].entries[j])) {
					// Copy to the front
					if(j != k) {
...
/*
 *JSP: This function implements an equality comparison for double floating point values.
 *JSP: Caution must be exercised with less than and greater than comparisons. since
 *JSP: it must be ensured that the values are not equal first.
 */
inline int DoubleEquals(double v1, double v2) {
	return fabs(v1 - v2) < 1E-12; //0.0000000001;
}

//inline int DoubleLessThan(double v1, double v2) {
//	if (DoubleEquals(v1, v2)) {
//		return 0;
//	} else if (v1 < v2) {
//		return 1;
//	} else {
