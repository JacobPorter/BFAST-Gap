...
{
	fprintf(stderr, "\n");
	fprintf(stderr, "BFAST-Gap:   the blat-like fast accurate search tool with run sensitive gapped alignment\n");
#ifdef GIT_REV
...
int main(int argc, char *argv[])
{
	if(argc < 2) return usage();
		...
		PrintError(PROGRAM_NAME, argv[1], "Unknown command", Exit, OutOfRange);
	}
	return 0;
}
